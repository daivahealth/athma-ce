...
async function bootstrap() {
  ...
  try {
    ...
    for (const [, moduleRef] of modules.entries()) {
      for (const [, wrapper] of moduleRef.controllers) {
        ...
        scanner.scanFromPrototype(instance, proto, (methodName) => {
          ...
          const paramTypes: any[] | undefined = Reflect.getMetadata('design:paramtypes', handler);
          if (!paramTypes) {
            issues++;
            ...
            logger.debug(
              `Missing design:paramtypes → controller=${proto.constructor?.name} method=${methodName} route=${ctrlPath}/${methodPath} requestMethod=${requestMethod}`
            );
          }
        });
      }
    ...
    logger.warn(`Detector failed: ${(e as Error).message}`);
  }
  // ---- /DETECTOR ----
  ...
  try {
    ...
    logger.error(`Swagger disabled due to error: ${(err as Error).message}`);
  }
  // ---- /SAFE SWAGGER ----
