...
import type { AbdmScope } from './abdm-credentials.service';

export type { AbdmScope };
...
export interface AbdmGateway {
  ...
  requestEnrolOtp(scope: AbdmScope, aadhaar: string): Promise<AbhaOtpChallenge>;
  ...
  requestLoginOtp(scope: AbdmScope, loginHint: string, loginId: string): Promise<AbhaOtpChallenge>;

  /** Step 2 of verifying an existing ABHA. */
  verifyLogin(scope: AbdmScope, txnId: string, otp: string): Promise<AbhaProfile>;

  /** Candidate ABHA addresses for a freshly enrolled account. */
  getAbhaAddressSuggestions(scope: AbdmScope, txnId: string): Promise<string[]>;

  /** Claims an ABHA address for a freshly enrolled account. */
  createAbhaAddress(scope: AbdmScope, txnId: string, abhaAddress: string): Promise<string>;
}
