...
app.get('/api/v1/pms/patients/:id', async (req, res) => {
    try {
        const result = await query('SELECT * FROM patients WHERE id = $1 AND is_active = true', [req.params.id]);
...
app.post('/api/v1/pms/patients', async (req, res) => {
    try {
        const { emirates_id, first_name, last_name, date_of_birth, gender, phone_number, email, address, city, emirate, nationality, blood_group, emergency_contact_name, emergency_contact_number, preferred_language = 'en' } = req.body;
        ...
        if (emirates_id) {
            const existingPatient = await query('SELECT id FROM patients WHERE emirates_id = $1', [emirates_id]);
...
app.post('/api/v1/pms/appointments', async (req, res) => {
    try {
        const { patient_id, staff_id, facility_id, start_time, end_time, appointment_type, reason, notes } = req.body;
        // Validate required fields
        if (!patient_id || !start_time || !end_time || !appointment_type) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        // Check if patient exists
        const patientCheck = await query('SELECT id FROM patients WHERE id = $1 AND is_active = true', [patient_id]);
...
startServer();
//# sourceMappingURL=pms-with-db.cjs.map