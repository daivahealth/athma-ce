...
let PatientRepository = class PatientRepository {
    ...
    async findByIdWithTranslations(id, client) {
        const prisma = this.resolveClient(client);
        return prisma.patient.findUnique({
            where: { id },
    ...
    async findByEmiratesId(emiratesId, client) {
        const prisma = this.resolveClient(client);
        return prisma.patient.findUnique({
            where: { emiratesId },
        });
    }
    async findByEmiratesIdAndTenant(emiratesId, tenantId, client) {
        const prisma = this.resolveClient(client);
    ...
    async getPatientAppointments(patientId, query, client) {
    ...
    async getPatientEncounters(patientId, query, client) {
        const { limit = 20, page = 1 } = query;
        const skip = (page - 1) * limit;
        const prisma = this.resolveClient(client);
    ...
    async getPatientDiagnoses(patientId, _client) {
    ...
    async getPatientMedications(patientId, _client) {
    ...
    async findDuplicates(patient, client) {
    ...
    async updateConsent(patientId, consentDto, client) {
        // Temporarily commented out until schema is updated
        throw new Error('Consent functionality not yet implemented in schema');
    }
    async getTranslations(patientId, client) {
        // Temporarily commented out until schema is updated
        return [];
    }
    async updateTranslations(patientId, translations, client) {
        // Temporarily commented out until schema is updated
