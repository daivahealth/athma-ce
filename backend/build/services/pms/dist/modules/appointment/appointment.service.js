    ...
    var AppointmentService = class {
        ...
        async createAppointment(createAppointmentDto) {
            ...
            if (conflicts.length > 0) {
                throw new common_1.ConflictException('Appointment conflicts detected', conflicts);
            }
            // Create appointment
        ...
        async checkInAppointment(id) {
            ...
            if (appointment.status !== 'confirmed') {
                throw new common_1.BadRequestException('Appointment must be confirmed to check in');
            }
            await this.appointmentRepository.update(id, {
                status: 'checked_in',
            });
        ...
        async completeAppointment(id) {
            ...
            if (!['checked_in', 'in_progress'].includes(appointment.status)) {
                throw new common_1.BadRequestException('Appointment must be checked in or in progress to complete');
            }
            await this.appointmentRepository.update(id, {
                status: 'completed',
                endTime: new Date(),
        ...
        async rescheduleAppointment(id, newStartTime, newEndTime, reason) {
            ...
            if (appointment.status === 'completed') {
                throw new common_1.BadRequestException('Cannot reschedule completed appointment');
            }
            ...
            const conflicts = await this.appointmentRepository.checkConflicts(conflictData, id);
            if (conflicts.length > 0) {
                throw new common_1.ConflictException('New time conflicts with existing appointments', conflicts);
            }
            await this.appointmentRepository.update(id, {
                startTime: newStartTime,
                endTime: newEndTime,
        ...
        validateAppointmentData(data) {
            ...
            if (durationMinutes !== data.duration) {
                throw new common_1.BadRequestException('Duration does not match start and end times');
            }
