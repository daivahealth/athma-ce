...
exports.createTenantIsolationMiddleware = createTenantIsolationMiddleware;
exports.withoutTenantCheck = withoutTenantCheck;
...
function createTenantIsolationMiddleware() {
    return async (params, next) => {
        ...
        if (!tenantId) {
            ...
            if (store && store.bypassTenantCheck) {
                return next(params);
            }
            throw new Error(`Tenant context required for ${model}.${params.action}. ` +
        ...
        switch (params.action) {
            ...
            default:
                ...
                console.warn(`Tenant isolation not implemented for operation: ${model}.${params.action}`);
        }
        return next(params);
    };
}
 ...
 */
function withoutTenantCheck(fn) {
    return shared_utils_1.RequestContext.run({
        ...shared_utils_1.RequestContext.getStore(),
