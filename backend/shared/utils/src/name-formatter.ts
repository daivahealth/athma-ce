...
export interface NameComponents {
  ...
  middleName?: string;
  lastName: string;
  prefix?: string; // Alternative to title (used in staff names)
}
...
export function formatName(
  ...
  template: string = '{title} {firstName} {middleName} {lastName}'
): string {
  ...
  Object.keys(nameData).forEach((key) => {
    const value = nameData[key as keyof typeof nameData];
 ...
 */
export const NAME_FORMAT_TEMPLATES = {
 ...
 */
export function parseFormatTemplate(configValue: string): string {
  try {
