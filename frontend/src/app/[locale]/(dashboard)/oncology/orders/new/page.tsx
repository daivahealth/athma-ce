...
export default function NewChemoOrderPage({ params }: { params: { locale: string } }) {
  ...
  const { data: carePlansData } = useOncologyCarePlans(cancerDiagnosisId ? { cancerDiagnosisId } : undefined);
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <Select value={cancerDiagnosisId} onValueChange={(v) => { setCancerDiagnosisId(v); setProtocolId(''); setOncologyCarePlanId(''); }}>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2">
              <Label>Notes</Label>
              <textarea className="w-full min-h-[60px] rounded-md border border-input bg-background px-3 py-2 text-sm" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Clinical notes, special instructions..." />
            </div>
          </CardContent>
        </Card>
