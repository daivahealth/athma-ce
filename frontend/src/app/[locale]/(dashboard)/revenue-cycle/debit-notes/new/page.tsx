...
import { useCreateDebitNote } from '@/modules/rcm/hooks/use-debit-notes';
import type { DebitNoteLine, CreateDebitNoteInput } from '@/modules/rcm/types/debit-note';
import type { Invoice } from '@/modules/rcm/types/invoice';
import { useResolveConfig } from '@/modules/foundation/hooks/use-configs';

const createLine = (lineNumber: number): DebitNoteLine => ({
  lineNumber,
  description: '',
  lineAmount: 0,
});
...
export default function NewDebitNotePage() {
  ...
  const [lines, setLines] = useState<DebitNoteLine[]>([createLine(1)]);
  ...
  const handleLineChange = (index: number, field: keyof DebitNoteLine, value: string) => {
    setLines((prev) =>
      prev.map((line, idx) => {
        if (idx !== index) return line;
        const updated: DebitNoteLine = { ...line };
        if (field === 'lineAmount') {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: CreateDebitNoteInput = {
      patientId: patientId.trim(),
      invoiceId: selectedInvoice?.id,
      debitNoteDate: debitNoteDate ? new Date(debitNoteDate).toISOString() : undefined,
      amount: totalAmount,
      currency,
    ...
    const created = await createDebitNote.mutateAsync(payload);
    toast({ title: 'Debit note created' });
    router.push(`/${locale}/revenue-cycle/debit-notes/${created.id}`);
  };
