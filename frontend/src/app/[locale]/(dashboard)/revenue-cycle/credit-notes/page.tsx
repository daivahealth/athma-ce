...
import { useCreditNotes } from '@/modules/rcm/hooks/use-credit-notes';
import type { CreditNote, CreditNoteStatus } from '@/modules/rcm/types/credit-note';
...
const createColumns = (locale: string, router: ReturnType<typeof useRouter>): ColumnDef<CreditNote>[] => [
  {
  ...
  },
  {
    ...
    header: 'Status',
    cell: ({ getValue }) => {
      const status = getValue<CreditNoteStatus>();
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${locale}/revenue-cycle/credit-notes/${row.original.id}`)}
      >
...
export default function CreditNotesPage() {
  ...
  const sanitized = useMemo(() => creditNotes ?? [], [creditNotes]);
  const filtered = useMemo(() => {
    ...
    return sanitized.filter((note) => {
      ...
      return `${note.creditNoteNumber} ${patientName} ${mrn} ${note.patientId}`
        .toLowerCase()
        .includes(term);
    });
  }, [sanitized, search]);
        ...
        <CardContent className="pt-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Button asChild className="ml-auto">
              <Link href={`/${locale}/revenue-cycle/credit-notes/new`}>
