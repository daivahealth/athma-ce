...
const FILTERS: { value: 'all' | EncounterStatus; label: string }[] = [
...
export default function TriageLandingPage() {
  ...
  const staffMap = useMemo(() => {
    ...
    (staffData?.data as StaffMember[] | undefined)?.forEach((staff) => {
      ...
      if (staff.id) map.set(staff.id, name || '');
    });
    return map;
  }, [staffData]);
  ...
  const renderStatus = (status: string) => (
    <Badge variant="outline" className={STATUS_BADGE[status] || 'bg-muted text-foreground'}>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use filters to focus on arrivals waiting for clinical triage.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {error && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {error instanceof Error ? error.message : 'Something went wrong – try again shortly.'}
              </AlertDescription>
            </Alert>
          )}
                    ...
                    <TableRow key={encounter.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          {encounter.startTime ? format(new Date(encounter.startTime), 'MMM d, yyyy h:mm a') : '—'}
                        </div>
                      </TableCell>
                      <TableCell>{renderStatus(encounter.status)}</TableCell>
                      <TableCell className="text-right">
                        <Button size="sm" onClick={() => handleOpen(encounter.id)}>
                          Open triage
                        </Button>
                      </TableCell>
                    </TableRow>
