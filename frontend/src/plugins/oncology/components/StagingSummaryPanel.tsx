        ...
        {stagings.map((staging: Record<string, unknown>) => (
          <div key={staging.id as string} className="border-l-2 border-primary pl-3">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              ...
              {staging.t_category ? ` | T${String(staging.t_category)}` : ''}
              {staging.n_category ? ` N${String(staging.n_category)}` : ''}
              {staging.m_category ? ` M${String(staging.m_category)}` : ''}
            </div>
            <div className="text-xs text-muted-foreground">
              {new Date(staging.staging_date as string).toLocaleDateString()}
              {Boolean(staging.is_recurrence) && (
