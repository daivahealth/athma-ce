...
## 1) Decision
Adopt **row-level multi-tenancy** in PostgreSQL with **RLS** enforced on all tables using a `tenant_id` column. One logical database per region (UAE) with:
...
## 3) Scope
- All PMS + RCM core tables including patients, appointments, encounters, claims, remittances, audit, documents (with S3 prefixes per tenant).
