...
```bash
# 20. Patients & Policies
psql -f 20-patients.sql
psql -f 21-policies.sql
psql -f 22-policy-benefits.sql
```

### Phase 6: Clinical Operations (Depends on Phases 3, 5)
```bash
# 23. Appointments & Encounters
psql -f 23-episodes.sql
psql -f 24-appointments.sql
psql -f 25-appointment-resources.sql
psql -f 26-encounters.sql
psql -f 27-encounter-links.sql
```

### Phase 7: Clinical Documentation (Depends on Phase 6)
```bash
# 28. Clinical Notes & Orders
psql -f 28-clinical-notes.sql
psql -f 29-orders.sql
psql -f 30-prescriptions.sql
psql -f 31-documents.sql
```

### Phase 8: Billing & RCM (Depends on Phase 6, 7)
```bash
# 32. Superbills & Claims
psql -f 32-superbills.sql
psql -f 33-superbill-items.sql
psql -f 34-claim-headers.sql
psql -f 35-claim-lines.sql
```

### Phase 9: Payments & Reconciliation (Depends on Phase 8)
```bash
# 36. Payments & Remittances
psql -f 36-patient-payments.sql
psql -f 37-remittance-headers.sql
psql -f 38-remittance-lines.sql
```

### Phase 10: Configuration & Rules (Depends on Phase 1-4)
```bash
# 39. Business Rules
psql -f 39-visit-classification-rules.sql
psql -f 40-visit-billing-map.sql
psql -f 41-followup-waiver-rules.sql
psql -f 42-visit-type-pricing-rules.sql
```
...
${PSQL_CMD} -f 20-patients.sql
${PSQL_CMD} -f 21-policies.sql
${PSQL_CMD} -f 22-policy-benefits.sql

# Phase 6
echo "Phase 6: Clinical Operations..."
${PSQL_CMD} -f 23-episodes.sql
${PSQL_CMD} -f 24-appointments.sql
${PSQL_CMD} -f 25-appointment-resources.sql
${PSQL_CMD} -f 26-encounters.sql
${PSQL_CMD} -f 27-encounter-links.sql

# Phase 7
echo "Phase 7: Clinical Documentation..."
${PSQL_CMD} -f 28-clinical-notes.sql
${PSQL_CMD} -f 29-orders.sql
${PSQL_CMD} -f 30-prescriptions.sql
${PSQL_CMD} -f 31-documents.sql

# Phase 8
echo "Phase 8: Billing & RCM..."
${PSQL_CMD} -f 32-superbills.sql
${PSQL_CMD} -f 33-superbill-items.sql
${PSQL_CMD} -f 34-claim-headers.sql
${PSQL_CMD} -f 35-claim-lines.sql

# Phase 9
echo "Phase 9: Payments & Reconciliation..."
${PSQL_CMD} -f 36-patient-payments.sql
${PSQL_CMD} -f 37-remittance-headers.sql
${PSQL_CMD} -f 38-remittance-lines.sql

# Phase 10
echo "Phase 10: Configuration & Rules..."
${PSQL_CMD} -f 39-visit-classification-rules.sql
${PSQL_CMD} -f 40-visit-billing-map.sql
${PSQL_CMD} -f 41-followup-waiver-rules.sql
${PSQL_CMD} -f 42-visit-type-pricing-rules.sql
